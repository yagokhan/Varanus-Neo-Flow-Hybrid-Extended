...
Extended: 33-asset universe, 4 scan TFs, group-aware thresholds.
...
class FastBacktestEngine:
    ...
    Group-aware thresholds applied at scan time.
    """
    ...
    def __init__(
        ...
        # Pre-build timestamp arrays for O(log n) lookups
        self._feat_idx = {}
        for asset, feat in features.items():
            self._feat_idx[asset] = feat["timestamp_ns"]

        # Cache group thresholds per asset
        # When no group_overrides, build GroupThresholds from BacktestParams globals
        # so Optuna trial params flow through correctly
        self._group_cache: dict[str, tuple[str, GroupThresholds]] = {}
        for asset in ASSETS:
            group = get_group(asset)
            if params.group_overrides:
                gt = get_thresholds(asset, params.group_overrides)
            else:
                # Map global BacktestParams → GroupThresholds for all groups
                gt = GroupThresholds(
                    min_confidence=params.min_pearson_r,
                    min_pvt_r=params.min_pvt_r,
                    combined_gate=params.combined_gate,
                    hard_sl_mult=params.hard_sl_mult,
                    trail_buffer=params.trail_buffer,
                    exhaust_r=params.exhaust_r,
                    pos_frac=params.pos_frac,
                    min_xgb_score=params.xgb_threshold,
                )
            self._group_cache[asset] = (group, gt)
    ...
    def _update_positions(self, current_1h_ns: int):
        ...
        for asset, pos in list(self.positions.items()):
            ...
            _, gt = self._group_cache.get(asset, ("B", get_thresholds(asset, None)))
            ...
            for idx in sub_indices:
                ...
                if pos.direction == -1 and bar_high >= pos.trail_sl:
                    to_close.append((pos, pos.trail_sl, bar_ts, "ADAPTIVE_TRAIL_HIT"))
                    break
                # Trend exhaustion (group-specific)
    ...
    def _scan_and_enter(self, current_1h_ns: int):
        """Fast scan: O(1) feature lookups + group-specific thresholds."""
        ...
        for asset in ASSETS:
            ...
            group, gt = self._group_cache.get(asset, ("B", get_thresholds(asset, None)))
            ...
            # Entry price
            best_tf = TF_FROM_IDX.get(int(feat["best_tf_idx"]), "1h")
    ...
    def run(
        ...
        logger.info(
            "FastBacktest complete: %.1fs, %d scans, %d trades",
