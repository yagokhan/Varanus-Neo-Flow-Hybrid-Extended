...
Extended: scans 4 timeframes (5m, 30m, 1h, 4h) instead of 3.
...
TF_LIST = ["5m", "30m", "1h", "4h"]   # Extended: 4 TFs
TF_TO_IDX = {"5m": 0, "30m": 1, "1h": 2, "4h": 3}
...
def _compute_atr_array(high, low, close, period=14):
    """Compute rolling ATR. Returns float64 array (NaN-padded)."""
...
def _precompute_asset(
    ...
    max_r_long = np.zeros(n_hours, dtype=np.float64)
    max_r_short = np.zeros(n_hours, dtype=np.float64)
    ...
    for tf_idx, tf in enumerate(TF_LIST):
        ...
        # For 4h, cap max period at available bars
        max_period = min(PERIOD_MAX, n_bars_tf - 1) if tf == "4h" else PERIOD_MAX

        for period in range(PERIOD_MIN, max_period + 1):
            ...
            log_windows = log_close[win_indices]
    ...
    for tf_idx, tf in enumerate(TF_LIST):
        ...
        max_period = min(PERIOD_MAX, len(pvt_full) - 1) if tf == "4h" else PERIOD_MAX
...
def precompute_all_features(
    ...
    logger.info("Pre-computing features for %d 1h timestamps across %d assets...",
                len(hourly_ns), len(all_data))
    ...
    try:
        ...
        logger.warning("joblib not found, falling back to sequential")
        features = {}
        for asset, asset_data in all_data.items():
            t1 = time.perf_counter()
            features[asset] = _precompute_asset(asset, asset_data, hourly_ns)
            logger.info("  %s: %.1fs", asset, time.perf_counter() - t1)
...
def save_features(features: dict[str, np.ndarray], output_dir: str) -> None:
    """Save pre-computed features to .npy files."""
